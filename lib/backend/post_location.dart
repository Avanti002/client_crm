...
void postinlocation(
  ...
  var request = http.Request('POST',
      Uri.parse('http://mobilecrm.erpdata.in/api/resource/Geo%20tagging'));
  request.body =
...
void postoutlocation(
  ...
  var request = http.Request('POST',
      Uri.parse('http://mobilecrm.erpdata.in/api/resource/Geo%20tagging'));
  request.body =
      '''{"outaddress":"$outaddress","outDate":"$outDate","outTime":"$outTime","outlat":"$outlat","outlong":"$outlong","outRunning":"$outRunning","total_km":"$totalkm","total_work":"$totalHours"}''';
// '''{"indate"="$inDate","intime"="$inTime","inlat"="$inlat","inlong"="$inlong","inaddress"="$inaddress","inrunning"="$inRunning","outdate"="$outDate","outtime"="$outTime","outlat"="$outlat","outlong"="$outlong","outaddress"="$outaddress","outrunning"="$outRunning"}''';
  request.headers.addAll(headers);
